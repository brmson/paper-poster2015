...
\begin{abstract}
	Question Answering is a sub-field of information retrieval
	and information extraction that is recently enjoying renewed
	popularity, triggered by the popular success of IBM Watson in
	the Jeopardy! competition.  But Question Answering research is
	now proceeding in several semi-independent tiers related to the
	...
	By our work, we seek to reunite and boost research efforts
	in Question Answering by providing a modular, open source
	pipeline for this task --- allowing integration
	various knowledge base paradigms,
	answer extraction and analysis strategies and using a machine
	learned model to order the answers.  Within this pipeline,
	we also supply a reference baseline QA system with solid
	performance and amenable to further extensions.
	In this paper, we review alternative open QA platforms,
	present the architecture of our pipeline,
	the components of the baseline QA system, and also analyze
	the system performance on a reference dataset.
\end{abstract}
...
\begin{keywords}
	...
	linked data, natural language processing, UIMA, software engineering.
\end{keywords}
...
We consider the Question Answering problem --- creating a function of
...
or personal assistants like Apple Siri, and it has been a natural
benchmark of general Artificial Intelligence progress with the high
...
domain-specific applications, though extending in this direction
...
be then reformulated as a task of translating free-text user query
...
portfolio of structured and unstructured datasets is then again a natural
further extension.
...
This makes the task noticeably harder as we index significantly less data and
search results may not be that well sorted, but we believe the result is a more
universal system that could be readily refocused on domain specific QA instead
of open domain trivia questions, and also a system more amenable for scientific
research as the results are fully reproducible over time.

Finally, a common restriction of the QA problem concerns selecting
the most relevant answer-bearing passage given a tuple of input question
and set of candidate passages. \cite{WangQAGrammar}
Such passage selection problem is certainly
...
question subjects, therefore has minimum fixed categories and hand-coded rules.
...
modules in YodaQA is one of our long-term plans.

\textbf{OpenQA} \cite{OpenQA} is a recent end-to-end QA pipeline platform
also developed independently during the course of our work, and shares our
goal to become a common research platform in the field.  However, the approach
is very different, as OpenQA seems more of a portfolio-based engine with
several independent pipelines which have their results combined, while YodaQA
emphasizes modularity on pipeline stage level, with e.g.\ all answer generators
sharing a common answer analysis stage.

The rest of this paper is structured as follows.  We outline the general structure
...
we distribute with our framework in Sec.~\ref{sec:yodaqarefpip}, and
show the performance contributions of individaul modules and overall
system performance in Sec.~\ref{sec:results}.  We conclude with a summary
of our work and outline of future extensions in Sec.~\ref{sec:conclusion}.
...
testbed and a practical system. The pipeline is implemented in Java,
using the popular Apache UIMA framework \cite{UIMA}.
Unlike OAQA, in YodaQA each artifact (question, search result, passage,
candidate answer) is represented as a separate UIMA CAS, allowing natural
parallelization and easy leverage of numerous pre-existing NLP UIMA components;
in contrast to OAQA, we put an emphasis at aggressive compartmentalization
of different tasks to interchangeable UIMA annotators rather than using it
just for a very high level pipeline flow and annotation storage.

The framework is split into several packages: \textbf{io} takes care
of retrieving questions and returning answers, \textbf{pipeline} contains
classes for the general pipeline stages, \textbf{analysis} contains
...
with the flow as in Fig.~\ref{fig:arch}, encompassing:
...
\item \textbf{Answer Production} generates candidate answers based on the question.
	Typically, this happens by performing a search (the \textbf{Primary Search}) in a knowledge base according to the question clues,
	and either directly using search results as candidate answers (especially in structured knowledge bases)
	or filtering relevant passages from these (the \textbf{Passage Extraction})%
\footnote{We have an infrastructure prepared for machine learning models scoring candidate passages,
		but they are not employed in the current version.}
	and generating candidate answers from these passages (the \textbf{Passage Analysis}).
...
\item \textbf{Answer Merging and Scoring} consolidates the set of answers, removing duplicates and using a machine learned classifier to score answers based on their features.
\item \textbf{Successive Refining} prunes the set of questions in multiple phases, interjecting some auxiliary tasks (evidence diffusion and gathering some additional evidence).
\end{itemize}
...
\textit{enwiki})
and structured (DBpedia \cite{dbpedia}, Freebase \cite{freebase}) datasets.

In our pipeline, we build upon existing third-party NLP analysis tools,
...
For practical examples of the pipeline contents in two particular cases,
see tables~\ref{tab:exEnder} and~\ref{tab:exIditarod}.


\subsection{Question Analysis}

During question analysis, we take the question text,
produce a part-of-speech tagging and dependency parse,
detect named entities and based on this,
as roughly inspired by the DeepQA system \cite{WatsonQuestion}
we generate several QA features: Clues, Focus, and LAT.

\textbf{Clues} represent keywords in the question that determine its content
and are used for generating candidate answers.
Clues based on different question components have different weight
(used in search retrieval and passage extraction, determined empirically) ---
in ascending other, all noun phrases, noun tokens and the selection verb;
the LAT (see below);
named entities;
...
its weight is boosted and is flagged as a \textbf{concept clue}.

\textbf{Focus} is the focus point of the question sentence
indicating the constraint of a subject that is being queried.
Six simple hand-crafted heuristics based on the dependency parse extract the focus.
``name of ---'' constructions are also traversed.
...
While these are unstructured per se, we take advantage of the organization of the \textit{enwiki} corpus
where entity descriptions are stored in articles that bear the entity name as title
and the first sentence is typically the most informative short description of the entity.
...
\textbf{Title-in-clue search} \cite{WatsonIR} searches the question clues in the article titles,
essentially looking for articles that describe the concepts touched in the question.
...
The document texts are split to sentences which are considered as separate passages
and scored based on sum of weights of clues occuring in the passage\footnote{%
The \textit{about-clues} which occur in the document title have their weight divided by four (as empirically determined).};
...
\textbf{Concept search} retrieves articles whose titles (or redirect aliases) exactly match clues in questions.
The first sentence and also passages extracted as in the full-text search are used for passage analysis.

Given a selected passage, the passage analysis process runs an NLP pipeline and generates candidate answers,
currently by simply converting all named entities and noun phrases in the passage to candidate answers.
Also, objects in sentences where subject is the question LAT are converted to candidate answers.
...
for each concept clue in the question, we query for all properties with this clue as a subject
and generate candidate answers for each object in such a triple, with the property label seeded as the LAT of the answer.

In particular, we query the DBpedia \texttt{ontology} (curated) and \texttt{property} (raw infobox)
namespaces and Freebase RDF export.
Due to performance reasons, we limit the number of queried Freebase topics to 5 and retrieve only 40 properties per each;
due to this limitation, we have manually compiled a blacklist of skipped
``spammy'' properties based on past behavior analysis
(e.g.\ location \textit{people\_born\_here} or music artist \textit{track}).

\subsection{Answer Analysis}

During the answer analysis, the system takes a closer look at the answer snippet
and generates numerous features for each answer.
The dominant task here is type coercion,
i.e.\ verification whether the answer type matches the question LAT.

The answer LAT may be generated by multiple strategies:
\begin{itemize}
	\item Answers generated by a named entity recognizer will have LAT corresponding to the triggering model;
		we use OpenNLP NER models \textit{date}, \textit{location}, \textit{money}, \textit{organization}, \textit{percentage}, \textit{person} and \textit{time}.
	\item Answers containing a number will have a generic \textit{quantity} LAT generated.
...
The generated features describe the origin of the answer (datasource, result score, clues of which type matched in the passage, distance-based score of adjacent clue occurences, etc.), syntactic overlaps with question clues and type coercion scores (what kind of LATs have been generated, whether any type coercion succeeded and what is the WordNet specificity).

\subsection{Answer Merge-and-Score}

The merging and scoring process also roughly follows the DeepQA approach \cite{WatsonScoring}.
...
To avoid overfitting, too rare features are excluded
...
a binary feature denoting whether a feature has not been generated
...
are weighed by $0.5/p$, similar to what the DeepQA team reported \cite{WatsonScoring}.
...
overfitting on the training set and we therefore do not include
successive refining in the benchmarked pipeline.


\section{Performance Analysis}
\label{sec:results}

We shall present performance analysis of our system.
First, we detail our experimental setup, which is designed such that
others can reproduce our results therefore precisely measure performance
gain provided by their extensions and strategies embedded in the system;
this also includes details on our question dataset.
...
We find this preferrable to classical IR measures like MRR or MAP
since in many applications only the single top answer output by the system
...
we use 4.6.0, and models of various NLP components which are brought
...
available in the documentation distributed along the source code.
...
This dataset is based on public question answering benchmark from
main tasks of the TREC 2001 and 2002 QA tracks
...
to a YodaQA predecessor by random internet users via an IRC interface.
This dataset was further reviewed, ambiguous or outdated questions were
removed and the regex patterns were updated based on current data.
We refer to the resulting 867 question dataset as \texttt{curated} and
randomly split it to the training and testing sets; randomly remaining
7 questions were left unused for now.
...
\footnote{For example numerical quantities with varying formatting and units are notoriously tricky to match.}
Second, during training the models are randomly initialized and therefore their final
performance on a testing set drifts a little.
...
MRR is the Mean Reciprocal Rank $|Q|\cdot\sum_{q\in Q}{1/r_q}$, time is the time spend answering one question.}
\end{table*}
...
Benchmark results over various pipeline configurations are shown in Fig.~\ref{tab:bench}.
...
(Please note that no computation pre-processing was done on the knowledge bases or datasets;
...
from the years we use are readily available at any rate.
...
Research described in the paper was co-supervised by Dr. Jan Šedivý and Dr. Petr Pošík %, FEE CTU in Prague
